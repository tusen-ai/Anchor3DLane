...
class RPN(nn.Module):
    ...
    def _make_layer(self, inplanes, planes, num_blocks, stride=1):
        ...
        block = []
        block.append(nn.ZeroPad2d(1))
        block.append(nn.Conv2d(inplanes, planes, 3, stride=stride, bias=False))
    ...
    def forward(self, x):
        ...
        if self.merge_batch:
            ...
            x = x.transpose(1, 2).flatten(0, 1)  # [BD, C, H, W]
        ups = []
        for i in range(len(self.blocks)):
            x = self.blocks[i](x)
            if i - self._upsample_start_idx >= 0:
                ups.append(self.deblocks[i - self._upsample_start_idx](x))
        if len(ups) > 0 and self.merge_out:
            ups = torch.cat(ups, dim=1)
        if self.merge_batch:
            if len(ups) > 0:
                for feat_idx in range(len(ups)):
                    ups[feat_idx] = ups[feat_idx].reshape(batch_size, depth, *ups[feat_idx].shape[-3:]).transpose(1, 2)  # [B, D, C, H, W] -> [B, C, D, H, W]
            else:
                ups = ups.reshape(batch_size, depth, *x.shape[-3:])  # [B, D, C, H, W]
                ups = ups.transpose(1, 2)  # [B, C, D, H, W]
        return ups
...
class RPN3D(nn.Module):
    ...
    def _make_layer(self, inplanes, planes, num_blocks, stride=1):
        ...
        block = []
        block.append(nn.ConstantPad3d(1, 0))
        block.append(nn.Conv3d(inplanes, planes, 3, stride=stride, bias=False))
        block.append(build_norm_layer(self._norm_cfg, planes)[1])
        block.append(nn.ReLU())
        # block = nn.Sequential(
        #     nn.ZeroPad2d(1),
        ...
        for j in range(num_blocks):
            block.append(nn.Conv3d(planes, planes, (1, 3, 3), padding=(0, 1, 1), bias=False))
            block.append(
                build_norm_layer(self._norm_cfg, planes)[1],
                # nn.BatchNorm2d(planes, eps=1e-3, momentum=0.01)
            )
